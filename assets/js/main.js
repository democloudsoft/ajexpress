...
// Defensive handler to prevent third-party browser extension errors (e.g. MetaMask, Web3 wallet auto-injections) from disrupting the application
window.addEventListener('unhandledrejection', function(event) {
  if (event.reason && (
    (typeof event.reason === 'string' && (event.reason.includes('MetaMask') || event.reason.includes('ethereum') || event.reason.includes('wallet'))) ||
    (event.reason.message && (event.reason.message.includes('MetaMask') || event.reason.message.includes('ethereum') || event.reason.message.includes('wallet')))
  )) {
